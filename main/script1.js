...
//     // const productId = heart.getAttribute("data-productid");
...
// welcomeConts
// JSON 데이터를 사용하여 특정 범위의 항목을 HTML 콘텐츠로 생성하는 함수
...
// 원하린 코딩
// JSON 데이터를 불러와 HTML 구조 생성
function generateWeeklyContent(data) {
  ...
  const targetIds = ["Week1", "Week2", "Week3", "Week4", "Week5", "Week6"];
...
// JSON 데이터 불러오기 및 HTML 구조 생성
fetch("/db.json")
  .then((response) => response.json())
  .then((data) => {
    generateWeeklyContent(data);
  })
  .catch((error) => console.error("JSON 로드 오류:", error));
