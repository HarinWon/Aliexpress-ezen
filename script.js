...
// GIF 파일 경로
const videos = [
  "aliExpressMainVideo1.gif", // 첫 번째 GIF 파일
  "aliExpressMainVideo2.gif", // 두 번째 GIF 파일
];

// 초기 이미지 설정
mainVideo.innerHTML = `<img src="./dbImg/${videos[0]}" alt="Main Video GIF">`;
...
arrows.forEach((arrow) => {
  arrow.addEventListener("click", (e) => {
    ...
    // mainVideo 요소의 HTML을 업데이트하여 GIF 변경
    mainVideo.innerHTML = `<img src="./dbImg/${videos[i]}" alt="Main Video GIF">`;
...
function setupEventHandlers(data, cartContArr, save) {
  ...
  document.addEventListener("click", (e) => {
    if (e.target.matches(".icon > a > img")) {
      ...
      if (selectedProduct) {
        const existingProductIndex = cartContArr.findIndex(
          (item) => item.id === selectedProduct.id
        );

        if (existingProductIndex > -1) {
          cartContArr[existingProductIndex].cart += 1;
        } else {
...
const gnbScrollWidth = categoryGnb.scrollWidth + categoryGnb.clientWidth / 4;
